...
# READ CSV FUNCTION
# =========================================================
_needed_cols_inf = set(['ts_date'] + target_columns + fault_columns)
...
def read_csv_day(filepath):
    ...
    try:
        df = pd.read_csv(
            filepath,
            encoding='utf-8-sig', sep=sep, decimal=',',
            usecols=lambda c: c.strip() in _needed_cols_inf,
    ...
    df_c = df.iloc[:N_TAKE].reset_index(drop=True)[['ts_date'] + target_columns + fault_columns].copy()
    print(f"    → {len(df_c)} titik")
    return df_c
...
norm4_g1     = norm_all.iloc[-4 * N_TAKE:].values
