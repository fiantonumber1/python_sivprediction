...
# Grid Search
WINDOW_CANDIDATES = list(range(1, 15))    # kandidat window size (jumlah hari input)
...
# ─────────────────────────────────────────────────────────────
# 8. EKSPOR LAPORAN KE TXT
# ─────────────────────────────────────────────────────────────
report_path = os.path.join(SCRIPT_DIR, "gridsearch_report.txt")
with open(report_path, "w", encoding="utf-8") as f:
    f.write(f"LAPORAN GRID SEARCH — TCN FORECASTER\n")
    f.write(f"Tanggal : {datetime.now():%Y-%m-%d %H:%M:%S}\n")
    f.write(f"Kandidat Window Size : {WINDOW_CANDIDATES}\n")
    f.write(f"Folds (TimeSeriesSplit) : {GS_FOLDS}\n")
    f.write(f"Epoch/fold             : {GS_EPOCHS}\n")
    f.write(f"Metrik utama           : {GS_METRIC.upper()}\n")
    f.write(sep + "\n")
    f.write(f"{'Rank':>4}  {'WS (hari)':>9}  {'N Sampel':>8}  {'RMSE':>10}  {'MAE':>10}  {'MAPE (%)':>10}  Keterangan\n")
    f.write("─" * 72 + "\n")
    for _, r in df_gs.sort_values('ws').iterrows():
        rk   = str(int(r['rank'])) if r['rank'] != '-' else '-'
        rmse = f"{r['rmse']:.6f}" if not np.isnan(r['rmse']) else "       N/A"
        ...
        mape = f"{r['mape']:.2f}" if not np.isnan(r['mape']) else "    N/A"
        mark = " ◄ TERBAIK" if int(r['ws']) == BEST_WS else ""
        f.write(f"{rk:>4}  {int(r['ws']):>9}  {int(r['n_samples']):>8}  {rmse:>10}  {mae:>10}  {mape:>10}{mark}\n")
    f.write(sep + "\n")
    f.write(f"\n★  WINDOW SIZE TERBAIK  : {BEST_WS} hari\n")
    f.write(f"   Konfigurasi          : input {BEST_WS} hari → prediksi 1 hari ke depan\n")
    f.write(f"   {GS_METRIC.upper()} terbaik (avg CV)  : {best[GS_METRIC]:.6f}\n")
    f.write(f"\nGunakan BEST_WS = {BEST_WS} pada model akhir (stage1_train_final.py)\n")
    f.write(sep + "\n\n")
    f.write("DETAIL PER FOLD\n")
    f.write("─" * 60 + "\n")
    f.write(f"{'WS':>4}  {'Fold':>4}  {'N Train':>7}  {'N Val':>5}  {'RMSE':>10}  {'MAE':>10}  {'MAPE (%)':>10}\n")
    f.write("─" * 60 + "\n")
    for _, r in df_fold.iterrows():
        mape_s = f"{r['mape']:.2f}" if not np.isnan(r['mape']) else "    N/A"
        f.write(f"{int(r['ws']):>4}  {int(r['fold']):>4}  {int(r['n_train']):>7}  {int(r['n_val']):>5}  "
                f"{r['rmse']:>10.6f}  {r['mae']:>10.6f}  {mape_s:>10}\n")

print(f"\n  gridsearch_report.txt       disimpan → {report_path}")
